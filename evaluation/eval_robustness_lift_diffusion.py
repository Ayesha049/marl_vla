...
def parse_arguments():
    parser = argparse.ArgumentParser(
        description="Evaluate lift policy robustness with action noise and filtering/diffusion methods"
    )
...
def save_results_to_csv(results, timestamp):
    """Save results to CSV."""
    csv_path = os.path.join(RESULTS_DIR, f"robustness_eval_lift_{timestamp}.csv")
