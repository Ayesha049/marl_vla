"""
Evaluate can policy robustness with action noise across multiple predefined noise levels.
...
Examples:
python eval_robustness_can.py --n_rollouts 100
python eval_robustness_can.py --epoch 500 --seed 42
"""
...
def parse_arguments():
    ...
    parser = argparse.ArgumentParser(
        description="Evaluate can policy robustness with action noise and filtering methods"
    )
    parser.add_argument('--epoch', type=int, default=600, help='Checkpoint epoch to load')
...
def save_results_to_csv(results, timestamp):
    ...
    """
    csv_filename = f"robustness_eval_can_{timestamp}.csv"
